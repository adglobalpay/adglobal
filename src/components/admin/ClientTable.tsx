...
const getSpecialBadges = (client: ApiClient) => {
  ...
  if (referredCount > 0) {
    if (referredCount >= 10) {
      badges.push({
        ...
        tooltip: `Ha referido a ${referredCount} clientes`
      });
  ...
  if (totalTransactions >= 20) {
    badges.push({
      ...
      tooltip: `${totalTransactions} transacciones`
    });
  }
...
export default function ClientTable({ limit }: Props) {
  ...
  useEffect(() => {
    async function loadClients() {
      try {
        ...
        setClients(limit ? data.slice(0, limit) : data);
      } catch (err: any) {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      ...
      case 'OK':
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[0.65rem] font-bold uppercase tracking-wider bg-emerald-50 text-emerald-700 border border-emerald-200">
      ...
      case 'PENDING':
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[0.65rem] font-bold uppercase tracking-wider bg-amber-50 text-amber-700 border border-amber-200">
      ...
      case 'REVIEW':
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[0.65rem] font-bold uppercase tracking-wider bg-red-50 text-red-700 border border-red-200">
      ...
      case 'DONE':
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[0.65rem] font-bold uppercase tracking-wider bg-blue-50 text-blue-700 border border-blue-200">
      ...
      default:
        return (
          <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[0.65rem] font-bold uppercase tracking-wider bg-slate-100 text-slate-700 border border-slate-200">
            {status}
          </span>
        );
    }
              ...
              <tr key={client.id} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="py-4 px-3 align-top w-48">
                  <div className="flex flex-wrap gap-1.5">
                    {badges.map((badge, index) => (
                      <div
                        key={index}
                        className={`inline-flex items-center px-2 py-0.5 rounded text-[0.65rem] font-bold border ${badge.color}`}
                        title={badge.tooltip}
                      >
                ...
                <td className="py-4 px-3 align-top">
                  <div className="space-y-1">
                    <span className="inline-block px-1.5 py-0.5 bg-slate-100 text-slate-600 rounded text-[0.65rem] font-black uppercase tracking-wider border border-slate-200">
                      {client.country}
                    </span>
                    <br/>
                    <span className="inline-block px-1.5 py-0.5 bg-slate-100 text-slate-600 rounded text-[0.65rem] font-black uppercase tracking-wider border border-slate-200">
                      {client.preferredMethod || 'N/A'}
                    </span>
                  </div>
                </td>
                  ...
                  <div className="flex flex-col items-end gap-2">
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      <a
                        ...
                        className="w-7 h-7 inline-flex items-center justify-center rounded-md bg-slate-50 text-slate-500 hover:bg-slate-100 hover:text-slate-800 transition-colors border border-slate-200 group relative"
                      ...
                      <a
                        ...
                        className="w-7 h-7 inline-flex items-center justify-center rounded-md bg-emerald-50 text-emerald-600 hover:bg-emerald-100 hover:text-emerald-700 transition-colors border border-emerald-200"
